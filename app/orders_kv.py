...
def list_active(ctx: Context) -> List[Dict[str, Any]]:
    return [o for o in _load(ctx).values() if o.get("status") in ("pending", "refund_pending")]

def mark_complete(ctx: Context, order_id: str, tx_hash: str | None = None, delivered_raw: int | None = None) -> None:
    orders = _load(ctx)
